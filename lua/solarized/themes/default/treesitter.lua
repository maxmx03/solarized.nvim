return function(c, config)
  ...
  set_hl('@preproc', { link = 'PreProc' }) -- various preprocessor directives & shebangs
  set_hl('@define', { link = 'Define' }) -- preprocessor definition directives
  ...
  set_hl('@float', { link = 'Float' }) -- floating-point number literals
  ...
  set_hl('@parameter', { fg = c.base0, italic = true }, { style = config.styles.parameters }) -- parameters of a function
  ...
  set_hl('@conditional', { link = 'Conditional' }) -- keywords related to conditionals (e.g. `if` / `else`)
  set_hl('@conditional.ternary', { link = 'Conditional' }) -- ternary operator (e.g. `?` / `:`)

  set_hl('@repeat', { link = 'Repeat' }) -- keywords related to loops (e.g. `for` / `while`)
  set_hl('@debug', { link = 'Debug' }) -- keywords related to debugging
  set_hl('@label', { link = 'Label' }) -- GOTO and other labels (e.g. `label:` in C)
  set_hl('@include', { link = 'Include' }) -- keywords for including modules (e.g. `import` / `from` in Python)
  set_hl('@exception', { link = 'Exception' }) -- keywords related to exceptions (e.g. `throw` / `catch`)
  ...
  set_hl('@storageclass', { link = 'StorageClass' }) -- modifiers that affect storage in memory or life-time
  ...
  set_hl('@symbol', { link = '@variable.builtin' }) -- symbols or atoms
  ...
  set_hl('@tag', { fg = c.blue, bold = true }) -- XML tag names
  set_hl('@tag.attribute', { fg = c.violet }) -- XML tag attributes
  set_hl('@tag.delimiter', { fg = c.base01 }) -- XML tag delimiters
